namespace DurableTask.PostgresSQL.Logging
{
    ...
    class LogEvents
    {
        ...
        {
            public GenericWarning(string details, string? instanceId)
            {
        ...
        internal class CheckpointStartingEvent : StructuredLogEvent, IEventSourceEvent
        {
            ...
            protected override string CreateLogMessage() =>
                $"{this.InstanceId}: Checkpoint starting. Status = {this.Status}.";
        ...
        internal class CheckpointCompletedEvent : StructuredLogEvent, IEventSourceEvent
        {
            ...
            protected override string CreateLogMessage() =>
                $"{this.InstanceId}: Checkpoint completed in {this.LatencyMs}ms.";
        ...
        {
            public DuplicateExecutionDetected(string instanceId, string? executionId, string name)
            {
                this.InstanceId = instanceId;
            ...
            protected override string CreateLogMessage() =>
                $"{this.InstanceId}: Duplicate orchestration execution detected and will be discarded: {this.ExecutionId}";
        ...
        internal class TransientDatabaseFailure : StructuredLogEvent, IEventSourceEvent
        {
            ...
            protected override string CreateLogMessage() =>
                string.IsNullOrEmpty(this.InstanceId) ?
                    $"Transient database failure (attempt {this.RetryCount}): {this.Details}" :
                    $"{this.InstanceId}: Transient database failure (attempt {this.RetryCount}): {this.Details}";
        ...
        internal class ReplicaCountChangeRecommended : StructuredLogEvent, IEventSourceEvent
        {
            ...
            protected override string CreateLogMessage() =>
                $"Replica count change recommended: current={this.CurrentCount}, recommended={this.RecommendedCount}";
